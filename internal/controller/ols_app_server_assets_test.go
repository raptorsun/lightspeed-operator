...
var _ = Describe("App server assets", func() {
	...
	Context("complete custom resource", func() {
		...
		It("should generate a configmap holding the additional CA certificates", func() {
			caConfigMap, err := r.generateAdditionalCAConfigmap(cr)
			Expect(err).NotTo(HaveOccurred())
		...
		It("should return error if the CA text is malformed", func() {
			originalCACerts := cr.Spec.OLSConfig.AdditionalCA
			defer func() {
				cr.Spec.OLSConfig.AdditionalCA = originalCACerts
			}()

			cr.Spec.OLSConfig.AdditionalCA = []string{"malformed certificate"}
			_, err := r.generateAdditionalCAConfigmap(cr)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("failed to decode additional CA certificate"))
		})
	})
